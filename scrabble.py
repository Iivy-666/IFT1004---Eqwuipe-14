...
class Scrabble:
    ...
    def determiner_gagnant(self):
        ...
        max_point = -1000
        gagnant = None
        for i in range(len(self.joueurs)):              # Compare le pointage de chaque joueur
            pt_joueur = self.joueurs[i].points
            if pt_joueur >= max_point:
                max_point = pt_joueur
                gagnant = i
        return self.joueurs[gagnant]
    ...
    def partie_terminee(self):
        ...
        if len(self.joueurs) < 2 or len(self.jetons_libres) == 0:
            return True
        return False
    ...
    def joueur_suivant(self):
        ...
        if self.joueur_actif is None:               # Si aucun joueur actif, choix aléatoire
            index_joueur = randint(0, len(self.joueurs) - 1)
            self.joueur_actif = self.joueurs[index_joueur]
        else:                                       # Si un joueur actif, on identifie le prochain
            index_joueur = self.joueurs.index(self.joueur_actif)
            self.joueur_actif = self.joueurs[(index_joueur + 1) % len(self.joueurs)]
    ...
    def changer_jetons(self):
        ...
        demander = True             #
        liste_position = []         # Position du chevalet a changer
        liste_est_valide = True
        while(demander):
            chaine_demander = input("Identifier les positions à changer et les séparer par un espace:")
            for caractere in chaine_demander[::2]:          # position à changer
                liste_position.append(int(caractere) - 1)   # -1 pour index du code 0-1
            for position in liste_position:
                if not self.joueur_actif.position_est_valide(position):
                    liste_est_valide = False
            if liste_est_valide is True:
                demander = False

        for position in liste_position:
            mon_jeton_retire = self.joueur_actif.retirer_jeton(position)
            self.jetons_libres.append(mon_jeton_retire)     # Remet dans le sac le jeton échangé
            mon_jeton = self.jetons_libres[0]
            self.jetons_libres.remove(mon_jeton)            # Retire le nouveau jeton du sac
            self.joueur_actif.ajouter_jeton(mon_jeton, position)
